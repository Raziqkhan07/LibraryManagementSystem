...
namespace LibraryMSMVC.Controllers
{
    ...
    {
        private Books1Entity bookDb = new Books1Entity();
        private TansEntity transDb = new TansEntity();

        // Returns admin request view, here admin can accept and reject the book requests
        public ActionResult Requests()
        {
            return View(transDb.tblTransaction1.ToList());
        }
        // Returns all book requests in json format.
        public ActionResult GetAllRequests()
        {
            var transactionList = transDb.tblTransaction1.Where(r => r.TranStatus == "Requested").ToList();
        ...
        {
            var transactionList = transDb.tblTransaction1.Where(r => r.TranStatus == "Accepted").ToList();
            return Json(new { data = transactionList }, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var transactionList = transDb.tblTransaction1.Where(r => r.TranStatus == "Returned").ToList();
            return Json(new { data = transactionList }, JsonRequestBehavior.AllowGet);
        }
