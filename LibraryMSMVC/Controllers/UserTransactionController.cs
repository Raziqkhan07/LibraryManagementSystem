...
namespace LibraryMSMVC.Controllers
{
    public class UserTransactionController : Controller
    {
        ...
        TansEntity transDb = new TansEntity();
        Books1Entity bookDb = new Books1Entity();
        ...
        public ActionResult Requested(int? userId)
        {
            ...
            tblUsers1 user = userDb.tblUsers1.Find(userId);
            if (user == null)
            {
                return HttpNotFound();
            }
            UserTransactionController.userId = (int)userId;
            var requestList = transDb.tblTransaction1.Where(s => s.TranStatus == "Requested" && s.UserId == userId);
        ...
        public ActionResult DeleteRequest(int? tranId)
        {
            ...
            {*/
                ...
                tblTransaction1 transaction = transDb.tblTransaction1.FirstOrDefault(t => t.TranId == tranId);
                if (transaction == null)
                {
                    return HttpNotFound();
                }
                tblBook1 book = bookDb.tblBook1.FirstOrDefault(b => b.BookId == transaction.BookId);
                book.BookCopies = book.BookCopies + 1;
                bookDb.SaveChanges();
                transDb.tblTransaction1.Remove(transaction);
                transDb.SaveChanges();
        ...
        public ActionResult Rejected(int? userId)
        {
            ...
            UserTransactionController.userId = (int)userId;
            var rejectedList = transDb.tblTransaction1.Where(s => s.TranStatus == "Rejected" && s.UserId == userId);
        ...
        public ActionResult Received(int? userId)
        {
            ...
            UserTransactionController.userId = (int)userId;
            var receivedList = transDb.tblTransaction1.Where(s => s.TranStatus == "Accepted" && s.UserId == userId);
