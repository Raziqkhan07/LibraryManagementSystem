...
namespace LibraryMSMVC.Controllers
{
    public class BorrowController : Controller
    {
        ...
        public ActionResult Borrow(int? bookId)
        {
            /*try
            {*/
                if (transDb.tblTransaction1.Where(t => t.UserId == userId).Count() < 6)
                {
                    if (bookId != null)
                    {
                        tblBook1 book = bookD.tblBook1.FirstOrDefault(b => b.BookId == bookId);
                        if (book == null)
                        {
                            return HttpNotFound();
                        }
                        if (book.BookCopies > 0)
                        {
                            ...
                            tblTransaction1 trans = new tblTransaction1()
                            {
                                BookId = book.BookId,
                                BookTitle = book.BookTitle,
                                BookISBN = book.BookSBN,
                            ...
                            bookD.SaveChanges();
                            transDb.tblTransaction1.Add(trans);
                            transDb.SaveChanges();
